...
namespace AdventOfCode2021.Day19
{
    public class Day19B : IDay
    {
        public void Run()
        {
            ...
            scanners.First().AssembleMapOfBeacons();

            // What is the largest Manhattan distance between any two scanners?
            int output = scanners
                .SelectMany(first => scanners
                    .Where(second => second != first)
                    .Select(second => (int)MathNet.Numerics.Distance.Manhattan(first.GetPosition(), second.GetPosition())))
                .Max();
