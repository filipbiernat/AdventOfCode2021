        ...
        public void Run()
        {
            ...
            foreach (Scanner lhs in scanners)
            {
                foreach (Scanner rhs in scanners)
                {
                    if (lhs != rhs)
                    {
                        double distance = Math.Abs(lhs.Position[0, 0] - rhs.Position[0, 0]) +
                            Math.Abs(lhs.Position[0, 1] - rhs.Position[0, 1]) +
                            Math.Abs(lhs.Position[0, 2] - rhs.Position[0, 2]);
                        distances.Add((int)distance);
                    }
                }
            }
            int output = distances.Max();
            Console.WriteLine("Solution: {0}.", distances.Max());
        }
    }
}
